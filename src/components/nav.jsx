...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-[100] transition-all duration-700 ${mobileMenuOpen ? 'bg-black' : scrolled ? 'bg-black/80 backdrop-blur-2xl border-b border-white/5' : 'bg-transparent'
      }`}>
      {/* Scroll Progress Bar */}
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="relative">
                <div className="w-8 h-8 sm:w-10 sm:h-10 border border-blue-500/30 rounded-lg flex items-center justify-center rotate-45 group-hover:rotate-90 transition-all duration-500">
                  <span className="text-white text-sm sm:text-base font-black -rotate-45 group-hover:-rotate-90 transition-all duration-500">D</span>
                </div>
              ...
              <div className="flex flex-col">
                <span className="text-sm sm:text-lg font-black tracking-[0.15em] sm:tracking-[0.2em] text-white leading-none">DHARANISH</span>
                <span className="text-[8px] sm:text-[10px] font-mono text-blue-500 tracking-[0.3em] sm:tracking-[0.4em] uppercase opacity-70">Portfolio</span>
              </div>
            </div>
          </a>
      ...
      <div className={`fixed inset-0 lg:hidden bg-black z-[99] transition-all duration-700 ease-in-out overflow-hidden ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'
        ...
        <div className="relative h-full flex flex-col p-6 sm:p-10 pt-24 sm:pt-32 overflow-y-auto">
          <div className="flex flex-col gap-5 sm:gap-8">
            {navLinks.map((link, index) => (
              <a
                ...
                href={link.href}
                onClick={(e) => { e.preventDefault(); handleClick(link.name, link.href); }}
                className={`group flex items-end gap-4 sm:gap-6 transition-all duration-500 ${mobileMenuOpen ? 'translate-x-0 opacity-100' : 'translate-x-20 opacity-0'
                  }`}
                style={{ transitionDelay: `${index * 100}ms` }}
              >
                <span className="text-blue-500 font-mono text-sm sm:text-lg opacity-40 group-hover:opacity-100 transition-opacity">0{index + 1}</span>
                <span className={`text-2xl sm:text-5xl font-black uppercase tracking-tighter ${activeSection === link.name.toLowerCase() ? 'text-white' : 'text-white/20'
                  }`}>
                  {link.name}
                </span>
              </a>
          ...
          <div className="mt-auto space-y-5 sm:space-y-8 pb-6">
            <a href="/Resume.pdf" download className="w-full py-3 sm:py-4 flex items-center justify-center gap-3 bg-white/5 border border-white/10 rounded-2xl text-white text-xs sm:text-sm font-black tracking-widest uppercase hover:bg-blue-600 transition-all duration-300">
            ...
            <div className="flex justify-between items-center">
              <div className="flex gap-6">
                <a href="https://github.com/DHARANISH45" target="_blank" rel="noopener noreferrer" className="text-white/40 hover:text-blue-500 text-xs sm:text-sm font-mono transition-colors">GH</a>
                <a href="https://www.linkedin.com/in/dharanish-sl/" target="_blank" rel="noopener noreferrer" className="text-white/40 hover:text-blue-500 text-xs sm:text-sm font-mono transition-colors">LI</a>
              </div>
